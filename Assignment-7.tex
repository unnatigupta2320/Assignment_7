...
\begin{align}
    ...
    \\
     \implies a&= \sqrt{41} 
     \\
    \text{Also, }\lambda_1&=\frac{\vec{u}^{\top}\vec{V}^{-1}\vec{u}-f}{41} \label{eqD}
\end{align}
\item The standard equation of ellipse is given by :
\begin{align}
\frac{\vec{y}^{\top}Dy}{\vec{u}^{\top}\vec{V}^{-1}\vec{u}-f}=1
\end{align}
\item Using \eqref{eqC} and \eqref{eqD} we get:
\begin{align}
\implies \frac{\vec{y}^{\top}\myvec{\lambda_1& 0 \\ 0 & \lambda_2}}{\vec{u}^{\top}\vec{V}^{-1}\vec{u}-f} =1
\\
\implies \vec{y}^{\top}\myvec{\frac{1}{41}& 0 \\ 0 & \frac{1}{16}}=1
\end{align}
